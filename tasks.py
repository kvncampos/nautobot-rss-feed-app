...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NTC_NEWS_FEED_xxx
namespace = Collection("ntc_news_feed")
namespace.configure(
    {
        "ntc_news_feed": {
            "nautobot_ver": "2.3.1",
            "project_name": "ntc-news-feed",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.ntc_news_feed.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.ntc_news_feed.compose_http_timeout,
        "NAUTOBOT_VER": context.ntc_news_feed.nautobot_ver,
        "PYTHON_VER": context.ntc_news_feed.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.ntc_news_feed.project_name}",
        f'--project-directory "{context.ntc_news_feed.compose_dir}"',
    ]

    for compose_file in context.ntc_news_feed.compose_files:
        compose_file_path = os.path.join(
            context.ntc_news_feed.compose_dir, compose_file
        )
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.ntc_news_feed.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.ntc_news_feed.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.ntc_news_feed.python_ver
    dockerfile_path = os.path.join(context.ntc_news_feed.compose_dir, "Dockerfile")
    base_image = (
...
def lock(
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.ntc_news_feed.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.ntc_news_feed.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.ntc_news_feed.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations ntc_news_feed"
...
@task(
    help={
        "version": "Version of Ntc News Feed to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml ntc_news_feed'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = (
        "coverage report --skip-covered --include 'ntc_news_feed/*' --omit *migrations*"
    )
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.ntc_news_feed.local):
